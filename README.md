...
3. Point the frontend at the local API by updating `app/config.js`:
...
This repo currently chooses the API by editing `app/config.js`.
...
- Password reset emails are generated by the API and currently contain a
  production site URL
- Most app pages talk directly to `API_ENDPOINT`, so `app/config.js`
  must match the backend you want to hit
